...
close(con)
rm(con)
twitter_df <- data.frame(line=1:length(lines), text=lines)
...
close(con)
rm(con)
news_df <- data.frame(line=1:length(lines), text=lines)
news_tidy <- news_df 
...
close(con)
rm(con)
blogs_df <- data.frame(line=1:length(lines), text=lines)
blogs_tidy <- blogs_df 
...
twitter_sample <- twitter_tidy[sample(nrow(twitter_tidy), 500000),]
news_sample <- news_tidy[sample(nrow(news_tidy), 77000),]
